...
- **Dynamic Persona Injection**: Active user profile (`professional_bio`, `core_skills`, `writing_style_rules`, `tone_of_voice`) is injected at runtime into generation prompts
- **Dynamic Brand Injection**: Active user brand profile (colors, font, style, offerings) is injected into caption/image prompt context
...
- **Clean Visual Output**: Generated/regenerated images are returned as-is with no automatic logo compositing
...
- **Professional CRM Data Table**: Airtable/Baserow-style table UI with sticky headers, sortable visual hierarchy, and high-density row review for fast prospect scanning.
- **Conversation-Aware Reply Generation**: Uses full cached thread history per `conversation_id` when generating outreach to avoid generic replies.
- **Per-Prospect Draft Persistence**: Every generated/manual draft is saved per CRM row so users do not lose work.
- **Minimalist Modal Footer**: Clean separation of tab actions and right-aligned action buttons without messy inline styles.

### 5. Voice Engine & LinkedIn Ingestion

- **LinkedIn ZIP ingestion**: Upload LinkedIn exports to build persona and knowledge context.
- **Structured knowledge extraction**: Gemini returns persona, brand positioning, and product knowledge as JSON.
- **RAG enrichment**: Structured knowledge chunks are stored in `voice_chunks` with category metadata.
- **CRM auto-population**: `messages.csv` threads are analyzed and saved into `crm_contacts` automatically.
- **Processing lock + auto-refresh**: UI disables generation actions during ingestion and refreshes when ready.

### 6. Brand Assets + Dynamic UI Theming

- **Firecrawl v2 Branding Extraction**: Deterministic extraction of colors, logo, and typography via `/v2/scrape` (`branding` format).
- **Full Palette + Font Surfacing**: Brand Assets UI displays all extracted website colors and detected font families (not only the editable core 3 colors).
- **LLM Theme Generation**: Gemini generates a complete UI token map (`ui_theme`) for dark dashboard surfaces (sidebar, panels, text, borders, glows, gradients).
- **Strict Core Color Enforcement**: `brand_primary` and `border_focus` are hard-pinned to Firecrawl primary color to prevent drift.
- **Live Theme Application**: `applyBrandToUI()` applies all `ui_theme` tokens as CSS variables, including RGB aliases used by hover/focus/glow states.
- **Manual Re-theme Support**: Editing primary/secondary/accent and clicking **Save & Apply** regenerates a synchronized `ui_theme` immediately.
...
- **brand-assets.js**: Brand extraction preview/save/reset flows, full palette/fonts rendering, and complete CSS-token theming application
- **style.css**: Modern dark theme with glassmorphism effects and **animated stepper progress UI**
- **crm-hub.js**: CRM filters + professional datatable rendering + per-contact draft modal/save flows
...
- `POST /api/preview-brand`: Extract brand assets from a URL (preview only, not persisted)
- `POST /api/save-brand`: Validate and persist brand assets for the active user
- `GET /api/brand`: Fetch current user's saved brand assets payload
- `POST /api/upload-linkedin`: Upload LinkedIn ZIP export for persona + CRM ingestion
- `GET /api/persona`: Fetch persona + ingestion status
- `POST /api/search-voice`: Retrieve relevant voice context for a topic
- `GET /api/crm/contacts`: Fetch CRM contacts with tag/warmth filters
- `POST /api/crm/generate-message`: Generate conversation-aware outreach and auto-save to row draft
- `PUT /api/crm/contacts/{contact_id}/draft`: Persist manual row-level CRM draft edits
...
- Passes resolved auth UID to `orchestrator.py` (`--user_id`) for tenant-scoped generation context
...
2. Load tenant persona + brand context from Supabase/local profile cache using `--user_id`
2. Build visual parts (images, video transcriptions)
3. Construct mega prompt with:
   - Active user persona block (highest-priority writing guidance)
   - Active user brand block (brand style + color context)
...
6. Return final_plan.json (no post-processing logo overlay step)
```
...
### Brand Assets Theming

1. Open the **Brand Assets** mode in the sidebar.
2. Enter a website URL and click **Analyze Website**.
3. Review:
   - Editable core colors (Primary / Secondary / Accent)
   - Extracted website palette chips (full list)
   - Extracted website fonts chips (full list)
4. Click **Save & Apply** to persist and apply theme globally.
5. Optional: manually tweak core colors and click **Save & Apply** again to regenerate synced theme tokens.
...
- **Output behavior**: Returns regenerated image directly (no automatic logo stamping/compositing)
...
### Brand Assets theming seems stale or still shows yellow accents

- Hard refresh browser cache (`Ctrl+Shift+R`) after frontend updates.
- Confirm latest asset query params are loaded (`style.css?v=...`, `script.js?v=...`, `brand-assets.js?v=...`, `crm-hub.js?v=...`).
- Re-run website analysis once to refresh old cached brand payloads lacking full extraction schema.
- Verify UI states now use theme RGB variables (`--brand-primary-rgb`) instead of hardcoded `rgba(249,199,79,...)`.
...
### Logo appearing on generated visuals unexpectedly

- Current system behavior: generation/regeneration pipeline does **not** composite logos onto visuals.
- If a logo still appears, it is part of the model-generated pixels from the prompt/content itself.
- Refine the prompt to explicitly avoid logos/watermarks/branding marks in the generated image.
